...
### 缩放行为（菜单"放大"/"缩小"按钮）
- 每次点击 ±0.1 倍，范围 0.5 ~ 2.0（带边界钳制）
- 基于各模型**基准 scale**（见各模型特效配置）与用户倍率相乘，例如 ninifashengri（基准 1.2）× 用户倍率 1.2 = 1.44
- 模型切换后缩放倍率保留（`switchTo` 内重新应用 `setScale`）
- 不持久化：刷新页面后重置为用户倍率 1（即恢复各模型默认基准 scale）

### 快捷切换指定模型（菜单"选择模型"按钮）
- 点击弹出选择面板，列出全部 5 个模型名称（当前模型高亮）
