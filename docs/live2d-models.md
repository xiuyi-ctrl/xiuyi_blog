...
本博客右下角 Live2D 看板娘由 `l2d-widget` 库驱动，共包含 6 个 Cubism 2 模型，支持模型切换、点击互动、摸头、隐藏/唤醒、拖拽。
...
- **无 hit area 的模型**（l_234400412 / l_234500311 / l_234200211）：任意位置点击均触发 `idle_click` 动作组（随机播放其中 1 个动作），实现见 `onPointerEnd` 兜底逻辑
...
### 快捷切换指定模型（菜单"选择模型"按钮）
- 点击弹出选择面板（显示在画布右侧、紧贴功能按钮，垂直居中），列出全部 6 个模型名称（当前模型高亮）
...
## 新增模型总览（l_234400412 / l_234500311 / l_234200211）

三个模型来自同一素材包（`D:\BaiduNetdiskDownload\models`），结构与行为完全一致：

| 项目 | 说明 |
|---|---|
| 类型 | Cubism 2（model.moc），本地托管 |
| 纹理 | l_234400412：3 张（texture_00~02）；l_234500211：2 张（texture_00~01）；l_234200211：3 张（texture_00~02） |
...
| 体积 | l_234400412：11.37MB；l_234500311：9.23MB；l_234200211：11.12MB |
...
## 6. l_234200211 模型（l_234200211/model.json）
